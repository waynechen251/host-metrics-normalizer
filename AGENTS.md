...
所有專案規格文件（spec）統一由 `.agents/` 目錄管理，禁止散落於其他位置。
...
- 新增或修改規格文件時，一律放在 `.agents/spec/` 之下，不要另開新的規格目錄。
- 新增其他專案或功能的規格時，於 `.agents/spec/` 下新增對應子目錄，並在 `index.md` 補上連結，不要把新規格塞進既有子目錄。
- 規格文件以外的一般文件（如部署說明、Grafana 使用說明）仍放在 `docs/`，不算規格文件。
- 若實作與規格文件不一致，應先更新 `.agents/spec/` 中對應的規格文件並與使用者確認，再進行程式碼變更，而不是讓程式碼與規格各自為政。
- 進行任何架構或行為變更前，必須先讀取 `.agents/spec/index.md` 並查閱對應的規格文件。
...
- 回答預設精簡、直接切入重點，避免不必要的前言、客套話、重複使用者已提供的資訊。
- 不主動輸出大段程式碼或整份檔案內容，除非使用者要求；改以檔案路徑＋行號、diff 摘要或重點描述表達變更。
- 說明/總結類文字控制在必要範圍內，不做逐步流水帳式敘述，除非使用者要求詳細說明。
- 工具呼叫結果不需逐一複述內容，只需摘要關鍵結論。
- 若使用者要求詳細/完整輸出，以使用者當次要求為準，不受此原則限制。
